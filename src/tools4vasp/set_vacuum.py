...
def set_vacuum(atoms_obj, vac, bottom_space=1, direction=2):
    ...
    translation_vector[direction] -= np.min(atoms_obj.positions[:,direction] - bottom_space)
    atoms_obj.translate(translation_vector)
    new_cell_vector = [0,0,0]
    new_cell_vector[direction] += np.max(atoms_obj.positions[:,direction]) + (vac - bottom_space)
    atoms_obj.cell[direction] = new_cell_vector
    return atoms_obj

def set_potcar_vacuum(poscar_path, vac, bottom_space=1, direction=2, overwrite=True, verbose=False):
    ...
    if verbose:
        print(f"Set vacuum to {vac} Angstroms in {poscar_path} (bottom space: {bottom_space} Angstroms, direction: {direction})")

def main():
    parser = argparse.ArgumentParser(
        prog="VacuumSetter",
        description="Tool for adjusting the vacuum of POSCAR files"
    )
    parser.add_argument("vacuum_size", type=float,
                        help="total cell length in the vacuum direction")
    ...
    parser.add_argument("-d","--direction", type=int, default=2,
                        choices=[0,1,2],
                        help="cell axis (0,1,2) to modify")
    parser.add_argument("-b","--bottom_space", type=float, default=1,
